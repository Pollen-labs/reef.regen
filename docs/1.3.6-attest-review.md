# Reef.Regen — Attestation Wizard

## Step 6: Review & Submit (Finalized Simple Flow)

**Purpose:**
Allow users to carefully review all submitted data before finalizing their attestation.
Once confirmed, the app performs a *single, atomic submission flow* —
upload → sign → relay → create DB record.
If the user closes the page before completion, no partial data is written to the backend.

---

## 🧭 Overview

### UX Summary

* Display all information from previous steps.
* Each section includes a ✏️ **edit icon** to navigate back to that specific step.
* Include the organization name (profile) as “submitted by.”
* Provide an optional **Internal ID** input for users’ own tracking.
* CTA: **“Looks good, submit”** triggers final submission.

---

## 🧩 UI Sections

| Section             | Source                                             | Editable?    | Notes                                   |
| ------------------- | -------------------------------------------------- | ------------ | --------------------------------------- |
| **Regen action(s)** | `reefRegenActions[]`                               | Yes (Step 1) | Display as colored chips                |
| **Action date**     | `actionDate` / `actionStart` → `actionEnd`         | Yes (Step 2) | Format `MM DD, YYYY` or `MM DD ~ MM DD` |
| **Site info**       | `siteName`, `siteType`, `siteDepthM`, `siteAreaM2` | Yes (Step 2) | 2-column grid                           |
| **Regen summary**   | `summary`                                          | Yes (Step 3) | Full paragraph                          |
| **Attachment**      | `fileName`                                         | Yes (Step 3) | Filename only                           |
| **Species**         | `species[]`                                        | Yes (Step 4) | Display as blue chips “Name Count”      |
| **Contributors**    | `contributors[]`                                   | Yes (Step 5) | Comma-separated                         |
| **Submitting as**   | `organizationName`                                 | No           | Read-only                               |
| **Internal ID**     | `internalId`                                       | Yes          | Editable local field                    |



## Layout & Components

**Title:** “Review your submission”
**Subtext:**

> “Please review your submission thoroughly as it will be permanently submitted to the blockchain and cannot be edited later.”

**Sections displayed:**

1. Regen Action(s)
2. Action Date & Site Info
3. Summary & Attachment
4. Coral Species
5. Contributors
6. Submission identity (their org profile name)
7. Internal ID (optional field)

**Footer:**

* **Back** button — returns to Step 5
* **Looks good, submit** button — triggers EAS submission process

Each section has a **pencil icon (✏️)** for quick edit → clicking it navigates back to the corresponding step.

Each section is rendered as a **card** with the following:

* Section title (e.g., “Regen action(s)”)
* Content from stored state (user selections, text, file, etc.)
* Top-right edit icon (navigates back)

Example:

```
[Regen action(s)]           [✏️]
Microfragmentation, Nursery
```

**Styling:**

* Uniform background for cards (slightly lighter shade than page background)
* Rounded corners, 2xl border-radius
* Hover state on edit icon (opacity + cursor pointer)




---

## 🧠 State Model (Zustand)

Extend the store minimally:

```ts
type WizardState = {
  // existing fields from steps 1-5...
  internalId?: string;
  submitting?: boolean;            // disables UI during submission
  submitPhase?: 'idle' | 'upload' | 'sign' | 'relay' | 'done' | 'failed';
  submitError?: string | null;
};
```

---

## ⚙️ Behavior

### Edit navigation

Each ✏️ icon triggers:

```ts
router.push(`/submit/steps/${stepNumber}`)
```

### Internal ID input

* Optional text (max 40 chars, `[A-Za-z0-9-_ ]`)
* Stored locally only; not required.
* Updated with debounce 250 ms:

  ```ts
  setPatch({ internalId })
  ```

---

## 🚀 Submission Flow (Simple Atomic Flow)

### 1️⃣ When user clicks **“Looks good, submit”**

* Lock the UI.
* Show a blocking overlay with a 3-step indicator:

  1. Uploading file
  2. Signing attestation
  3. Broadcasting to network
* Add `beforeunload` handler warning if they attempt to close the page.

### 2️⃣ Upload file (if any)

```ts
if (state.fileBlob) {
  const { cid, url } = await uploadToIPFS(state.fileBlob);
  setPatch({ fileCid: cid, fileUrl: url });
}
```

* On failure → show toast “Upload failed, please retry.” Stop submission.
* If user closes tab before this finishes → no DB writes, safe cancel.

### 3️⃣ Sign & relay to EAS

```ts
const easUID = await signAndRelayToEAS(buildEASPayload(state));
```

* On wallet rejection or network failure → show toast “Signing cancelled / failed.”
* No DB writes yet.

### 4️⃣ Create DB record (only after EAS UID exists)

```ts
await api.post('/api/attestations', {
  ...buildDBRecord(state),
  eas_uid: easUID,
});
```

* The backend **must require** `eas_uid` to insert — this prevents orphan drafts.
* Server validates UID + schema + signer.

### 5️⃣ Complete

* Reset store (`reset()`).
* Redirect → `/submit/success?uid=<easUID>`.

---

## 🧯 Error Handling

| Stage                   | User Message                                   | Recovery                         |
| ----------------------- | ---------------------------------------------- | -------------------------------- |
| Upload failed           | “Upload failed. Please retry.”                 | Retry button re-calls upload.    |
| Signing rejected        | “Signing cancelled — submission not sent.”     | Return to Review screen.         |
| Relay timeout           | “Network busy. Try again.”                     | Retry re-runs relay only.        |
| DB create failed (rare) | Save `pendingFinalize = { eas_uid, snapshot }` | Prompt on next load to finalize. |

---

## 🔒 Blocking overlay UX

During submission:

| Phase    | Display text                                              |
| -------- | --------------------------------------------------------- |
| `upload` | “Uploading your file to IPFS… Please keep this tab open.” |
| `sign`   | “Requesting attestation signature…”                       |
| `relay`  | “Broadcasting to network…”                                |
| `done`   | “Submission complete.”                                    |

---

## Addendum — Network Auto‑Switch behavior (2025‑11‑03)

- We added an early network alignment step that requests `wallet_switchEthereumChain` as soon as the embedded wallet provider is connected. This reduces confusion where users had to click “Confirm & Submit” twice (first to switch networks, then to sign).
- Important: wallets still require user consent for switching networks. This change only moves the prompt earlier; it does not make switching silent.
- The target chain is derived from `NEXT_PUBLIC_CHAIN_ID` (defaults to OP Sepolia 11155420). If the chain is not present, we first request `wallet_addEthereumChain` and then switch.
- You can disable this behavior by setting `NEXT_PUBLIC_AUTO_SWITCH_ON_CONNECT=false`.
- The attestation form remains untouched here to preserve historical context; the change is implemented at the provider layer so the submit flow remains the same once the wallet is on the correct network.

Include spinner + progress bar; disable all navigation.

---

## 🗃️ Backend contract

**POST `/api/attestations`**

```json
{
  "organization_name": "Coral Guardians Initiative",
  "reef_regen_actions": ["Microfragmentation", "Nursery"],
  "site": {
    "name": "East side coral",
    "type": "Nursery",
    "depth_m": 3,
    "area_m2": 167,
    "location": [99.842, 10.113]
  },
  "action_date": "07-07-2025",
  "summary": "Coral reef regeneration involves nurturing coral fragments...",
  "file": {
    "name": "filename.pdf",
    "cid": "bafybeihashcidexample",
    "url": "https://gateway.filebase.io/ipfs/bafybeihashcidexample"
  },
  "species": [
    { "name": "Heterogorgia tortuosa", "count": 31 },
    { "name": "Acanella arbuscula", "count": 52 }
  ],
  "contributors": ["Erick Palau", "James Creek", "Sarah Bolin"],
  "internal_id": "CORAL-113",
  "eas_uid": "0x…"
}
```

**Server must:**

* Reject if `eas_uid` is missing.
* Verify UID and signer.
* Insert only on valid attestation.
* Return `201 Created`.

---

## 🧩 State resets after success

```ts
reset();          // clear persisted wizard
router.replace(`/submit/success?uid=${easUID}`);
```

---

## ✅ QA Checklist

* All data from steps 1–5 displayed correctly.
* Edit icons navigate to correct steps.
* Internal ID persists locally.
* Submit button triggers upload → sign → relay → create → redirect.
* Closing tab during submission cancels safely (no orphan DB data).
* Success screen shows EAS UID + “View on EAScan” link.

---

## 💡 Summary

| Feature           | Design choice                          |
| ----------------- | -------------------------------------- |
| Resume drafts     | ❌ None (keep it simple)                |
| Partial DB writes | ❌ Prevented                            |
| IPFS upload       | ✅ Client-side only                     |
| Signing flow      | ✅ Background; user sees progress bar (TBD)  |
| Final record      | ✅ Created **only** after valid EAS UID |
| Recovery          | ✅ Optional finalize if DB write fails  |




# Important Notes — Network & Relay

- Auto network switch
  - Detects current chain via wallet/viem and switches to the expected chain (`NEXT_PUBLIC_CHAIN_ID`, Optimism Sepolia 11155420) using `walletClient.switchChain({ id })` before any on‑chain reads.
  - If switching fails, the user sees a clear wrong‑network error and submission stops.

- EAS contract preflight
  - Verifies `NEXT_PUBLIC_EAS_ADDRESS` has bytecode on the active chain. If not, submission stops with a clear message (prevents “getNonce returned 0x” errors).

- Delegated signing (EAS SDK)
  - Encodes data with SchemaEncoder for this schema:
    `string organizationName,string[] reefRegenAction,string actionDate,string siteName,string siteType,string[] location,string locationType,string srs,uint256 siteDepthM,uint256 siteAreaSqM,string actionSummary,string[] biodiversity,string[] contributors,string fileName,string ipfsCID`.
  - Uses `eas.getDelegated().signDelegatedAttestation` with:
    - `recipient`: connected wallet (or provided override)
    - `expirationTime`: no expiration
    - `revocable`: true
    - `refUID`: zero bytes32
    - `value`: 0n
    - `nonce`: read from `eas.getNonce(attester)`
    - `deadline`: now + 30 minutes (seconds since epoch) to avoid slow‑network expiries
  - Ensures the outgoing payload explicitly carries `message.deadline` and `message.nonce` values (matches AttestationForm behavior).

- Relay payload (what the worker expects)
  - Client POSTs to `/api/relay` (server proxy → `RELAYER_URL`) with:
    `{ attester: 0x..., delegatedAttestation: { domain, types, primaryType: 'Attest', message, signature } }`
  - Bigints are serialized to strings by the client. Worker must coerce to bigint safely.
  - Worker validates schema, nonce, signature, and deadline; then relays `attestByDelegation` and returns `{ uid, txHash }`.
  - Typical error codes: `BAD_NONCE`, `DEADLINE_EXPIRED`, `INVALID_SIGNATURE`.

- Upload → Sign → Relay → Finalize (atomic)
  - File (if any) uploads to IPFS first; CID + gateway URL stored locally.
  - No DB writes until after relay returns a valid `uid`.
  - Finalize API requires `eas_uid` and inserts the attestation + joins; prevents orphan drafts by design.

- Success & reset
  - On success: reset the wizard store and redirect to `/submit/success?uid=<uid>`.


# Reef.Regen — Attestation Wizard

## Step 6: Review & Submit

**Goal:**
Let users review all information they entered across previous steps before final submission to the blockchain (EAS).
They can revise specific sections, add an optional internal ID (for their own record-keeping), and finally confirm submission.
Once submitted, data becomes immutable on-chain.

---
