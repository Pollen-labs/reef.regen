...
* [ ] Deploy updated EAS schema including `fileCID`.
* [ ] Add new columns in Supabase: `file_cid`, `file_gateway_url`, `show_on_map`.
* [ ] Setup Filebase credentials and bucket (`regen`).
* [ ] Implement server util to generate gateway URL from CID (configurable base gateway).
* [ ] Test Filebase upload API (signed requests).

### **Phase 2 — Frontend Integration (Week 2)**

* [ ] Add file upload UI to attestation form (image/PDF).
* [ ] Upload to Filebase; on success, capture CID and compute gateway URL.
* [ ] Auto-fill CID into EAS payload; persist gateway URL in DB record.
* [ ] Show upload progress, success, and error states.

### **Phase 3 — Relayer & EAS Update (Week 3)**

* [ ] Update relayer to accept payloads that include `fileCID` (ignore gateway URL).
* [ ] Confirm EAS typed data with `fileCID` passes validation.
* [ ] End-to-end test: form → upload → sign → relayer → on-chain; DB stores URL.

### **Phase 4 — Map & Profiles (Week 4)**

* [ ] `/map` only displays `show_on_map = TRUE`.
* [ ] Profile and map popovers show a "View file" link if `file_gateway_url` present.
* [ ] Admin/Supabase UI toggle for `show_on_map` per attestation.

### **Phase 5 — QA & Deployment (Week 5)**

* [ ] Verify gateway links open correctly across devices.
* [ ] Verify IPFS retrieval via CID and via gateway URL.
* [ ] Deploy updated relayer and Next.js app.
