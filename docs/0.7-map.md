# 0.7 – Map Sprint: Global Navigation + Map Experience
...
* **AppShell** — sticky TopNav, content area, Footer.

  * Slots: `topNav`, `content`, `footer`
  * Container: map page uses full-bleed; other pages (future) can use `max-w-7xl`.
  * Z‑layers: Map (z0), Location Pane (z10), Attestation Modal (z20), Toasts (z30).
...
* **TopNav**
  ...
  * Behavior: sticky, dark theme; mobile hamburger → Sheet.
* **Footer**
...
1. **MapView**
   ...
   * Interactions: click pin → `onSelect`; ESC → deselect; optional URL sync.
...
3. **LocationPane** (left drawer)
   ...
   * Sections:
     ...
     * Header: org/location name (hd‑4), coordinates.
     * **Stats** card: Regen actions donut + legend.
     * **Species**: pills list.
     * **AttestationList**: rows → open modal.
   * Mobile: full-height sheet.

4. **AttestationItem** (row in pane)

   * Content: date, overflow actions; click → open modal.
...
> **Why "contracts"?** Because both the UI and the data layer “sign” this agreement. Mock data adheres to these types today; APIs should return data that matches them tomorrow. This keeps components stable and reduces refactors.

---

## URL State (nice to have P1)

* `?loc=<id>` — opens LocationPane.
* `?att=<id>` — opens AttestationDetailModal (implies `loc`).
...
* [ ] TopNav implemented (desktop + mobile)
* [ ] Footer implemented
* [ ] MapView full-bleed + responsive
* [ ] Marker select/deselect
* [ ] LocationPane sections populated from mocks
* [ ] Attestation modal with focus trap + close flows
...
> **Note:** Although the Figma design references fixed dimensions, this app will be built **responsive‑first** using Tailwind's responsive utilities (`sm:`, `md:`, `lg:`) and fluid units (e.g. `max-w`, `flex`, `grid`, `aspect-video`). Each component here should gracefully adapt to small screens (mobile drawers, stacked layouts) and large viewports.

Below are paste‑in skeletons aligned to **0.6/0.7 tokens** (Outfit, tracking, colors) and the **Map Sprint**. Replace mock data as you wire APIs.
    ...
    <header className="sticky top-0 z-40 bg-black text-white">
      <div className="px-8 py-6 flex justify-between items-end">
        ...
        <a href="/map" className="flex items-center gap-1">
          ...
          {/* Wordmark placeholder block – swap for SVG/image asset */}
  ...
  return (
    <footer className="bg-vulcan-800 flex flex-col gap-8">
      ...
      <div className="flex w-full justify-center items-center gap-5 overflow-hidden">
        {/* Three repeating coral‑like blocks (simplified) */}
        {[0,1,2].map(i => (
...
export default function MapPage() {
  // mock state — replace with data hooks
...
### ✅ Phase 1: Global Navigation (Complete)
...
### ✅ Phase 2: Map Page Foundation (Complete)
...
- **MapView Component** ([components/map/MapView.tsx](../components/map/MapView.tsx))
  ...
  - Interactive: click pin → `onSelect`, ESC → deselect
...
### 🚧 Phase 3: Interactive Components (Next)
