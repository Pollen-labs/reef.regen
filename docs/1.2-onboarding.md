Absolutely — here’s the **final refined product spec** for your Reef.Regen onboarding flow, including corrected copy, best-practice naming (`ProfileSetupStep` / `ProfileDetailsStep`), and improved accessibility-friendly markup for dev handoff:

---

# Reef.Regen — Onboarding Flow Spec
...
- “Profile name” in earlier drafts = `org_name` in our database and APIs.
- We will use the term “organization name” in UX copy and persist it as `org_name`.
...
| ---- | ---------------------- | ------------------------------------------------------------------------------ |
| 1    | **ProfileSetupStep**   | Collect and save the user’s organization name (required)                       |
...
1. **On Sign In**
   ...
   * If empty → trigger onboarding flow.
   * If present → skip onboarding and continue to intended destination.
...
3. **Entry Point Redirects**
   ...
   * If user signed in from **Submit** page → return to Submit after onboarding.
   * If user signed in from **Home** → return to Home.
...
| -------------- | --------- | -------- | ------------------------------------ |
| `org_name`     | `text`    | ✅        | Used in EAS attestations             |
| `description`  | `text`    | ❌        | Optional text for public profile     |
| `website`      | `text`    | ❌        | Optional link for public profile     |
...
* Onboarding completion → `router.replace(referrer || "/")`
* Store referrer path via `localStorage` or router query (`?redirect=/submit`)
* No separate redirect page — redirect triggers automatically on successful save
...
* User refreshes mid-onboarding → restore last step
* Existing profile name → skip flow
* Invalid website format → inline validation error
...
* `ProfileSetupStep` → replaces “onboarding-1”
* `ProfileDetailsStep` → replaces “onboarding-2”
    ...
    <div class="flex flex-col gap-8">
      ...
      <div class="flex flex-col gap-2">
        ...
        <textarea id="description" name="description" rows="6"
                  class="w-full p-4 bg-Vulcan-700 rounded-lg text-White placeholder-Vulcan-400 text-2xl font-light leading-9
                         focus:outline-none focus:ring-2 focus:ring-Orange"
                  placeholder="Tell us about your organization’s work..."></textarea>
      </div>
...
async function handleSetup(values, walletAddress) {
  ...
  router.replace({ pathname: '/profile/setting', query: { redirect: redirectTo } });
}
...
| `onboarding_skip_optional`  | –                                  | Skipped optional fields   |
...
* [ ] Mobile viewport verified at 375 × 812 and 320 × 640
