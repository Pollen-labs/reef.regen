...
- Relayer health (proxied): `/api/relay` → returns `{ eas, schema, chainId }`
...
| depthScaled         | uint256    | Depth in meters ×100 (e.g., 3.50 → 350)          |
| surfaceAreaScaled   | uint256    | Area in m² ×100 (e.g., 12.34 → 1234)             |
...
- POST `RELAYER_URL` (proxied via Next: `POST /api/relay`)
  ...
  - Verifies signature → `attestByDelegation` → returns `{ uid, txHash }`

Next.js API (server → Supabase)
- POST `/api/profiles/upsert` → `{ wallet_address, org_name?, website?, description?, handle? }`
- GET  `/api/profiles/by-wallet?address=0x…` → `{ handle | null }`
- POST `/api/attestations/create` → create draft (date/lat/lng required)
- POST `/api/attestations/set-uid` → set `{ uid }` after successful relay
- GET  `/api/attestations/public` → GeoJSON FeatureCollection for MapLibre
...
| ------- | ------------------- | ------------------------------------ |
| Home    | `/`                 | Overview + CTA to Submit             |
| Submit  | `/submit/steps/1`   | Multi‑step wizard for attestations   |
| Attest  | `/attest`           | Legacy POC delegated form (kept)     |
...
### **Phase 1 — Core Infrastructure and backend** — ✅ Completed
### **Phase 2 — Prepare EAS schema** — ✅ Completed (OP Sepolia)
### **Phase 3 — Frontend Foundations** — ✅ Completed
### **Phase 4 — Relayer Integration** — ✅ Completed
### **Phase 5 — Frontend integration** — ✅ Completed
### **Phase 6 — Visualization & Profiles** — ✅ Completed (baseline)
### **Phase 7 — QA & Launch** — ▶ In progress
