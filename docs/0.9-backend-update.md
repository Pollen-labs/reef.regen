...
The database stores a PostGIS point and normalized lon/lat only. Location Protocol JSON is constructed client-side when needed.

Fixed values (frontend):
...
- `lon` numeric(9,6), `lat` numeric(9,6)
...
| (removed)          |                                   | Location payload removed in v0.4; JSON is constructed in frontend |
...
To simplify frontend queries and avoid repeated joins, v0.4 adds:

- `public.site_points_v` — One row per site with `attestation_count` (counts only `show_on_map = true`). Used by the Map page pins via `/api/attestations/public`.
- `public.attestation_min_v` — Per‑attestation mini aggregate with `action_types[]` and `species[]` arrays. Used by the site detail RPC and attestation detail API.
- `public.get_site_detail(site_uuid, recent_count=5)` — Returns a single JSON document for the Location Pane (site name, org, coordinates, totals, actions breakdown with colors, species, recent items).

Views are read‑only helpers; RLS still applies to the underlying tables they read from.
