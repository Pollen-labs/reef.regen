# Reef.Regen — Profile / Settings Specification
...
It’s only visible to the authenticated user viewing their own account.

The page is divided into three sections:

1. **Basic Information** — profile metadata editable anytime.
2. **Sites** — manage owned reef restoration sites.
3. **Security** — wallet info, private key export, and logout.
...
| --------------------- | ----------- | ------------------------------------------ | ------------------------------------------------- |
| **Organization name** | Text        | 2–80 chars                                 | Required                                          |
| **Description**       | Multiline   | ≤ 500 chars                                | Optional                                          |
| **Website**           | URL         | Must start with `http://` or `https://`    | Optional                                          |
| **Handle**            | Text (slug) | Unique, lowercase, 3–32 chars, `[a-z0-9-]` | Required, must be unique across `profiles.handle` |

### Behavior

* Inputs are **editable inline**.
* “Save” button is disabled if no changes are made.
* “Cancel” restores last saved data.
* On save:
  ...
  * Shows toast: “Profile updated successfully.”
...
* If unavailable → red text warning: “Handle already taken.”
...
Reuses the **Step 2 “Edit site” modal** component for add/edit operations.
...
* Existing sites listed in a simple list:
  ...
  [Edit Icon]
  ```
* “Add a site” button opens the Map modal (same as submission flow).
...
| **Delete Site (optional)** | Small trash icon → confirmation → `DELETE /api/sites/:id`.                                       |
...
See reference: [Metamask Embedded Wallet — Key Export](https://docs.metamask.io/embedded-wallets/dashboard/advanced/key-export)
...
3. If enabled, show modal:
   ...
   > “Export your wallet key (use with caution).”
...
6. Show warning toast:
   ...
   > “Your private key has been downloaded. Keep it secure.”

#### Safety notice

* Show inline warning message below button:

  > “⚠️ Do not share your private key. Anyone with this key can access your funds.”
...
* [ ] “Export private key” triggers download with correct data.
...
│  ├─ DescriptionTextarea
│  ├─ WebsiteInput
│  ├─ HandleInput (with uniqueness check)
│  └─ Save/Cancel buttons
├─ SiteManager
│  ├─ SiteList
│  ├─ AddSiteButton → SiteModal
├─ SecuritySection
│  ├─ WalletDisplay
